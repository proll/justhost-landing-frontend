jhst.TW = Backbone.Model.extend({
	...
	successToken: function(response, status, xhr){
		...
		if (resp.error){
			...
			this.loading = false;
			jhst.log("WHP/auth/TW : get twitter request token = ["+resp.oauth_token+"]");
			this.req_count = 0;
	...
	fetch: function(user_obj){
		jhst.log("GET TW AUTH = ["+user_obj.uid+" : "+user_obj.token+" : "+user_obj.secret+"]");
		jhst.log("WHP/auth/TW : get auth...");
