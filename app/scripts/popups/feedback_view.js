jhst.FeedbackView = Backbone.View.extend({
	...
	submit: function (e) {
		...
		// validation
		if (_.isEmpty(name)){
			this.showError(jhst.localize('noname', 'feedback'), 'name')
		}else if (_.isEmpty(phone)){
			this.showError(jhst.localize('Please fill the phone field', 'feedback'), 'phone')
	...
	showError: function(txt, input_name) {
		...
		switch(input_name) {
			...
			case 'name': 
				...
				break;
			case 'phone': 
				this.$input_phone.parents('.jhst__inp-cont').toggleClass('error-inp', true);
