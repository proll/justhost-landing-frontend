jhst.AuthView = Backbone.View.extend({
	...
	render: function(){
		...
		this.popup_view = new jhst.PopupView({klass: "auth-popup"});
	...
	submit: function (e) {
		...
		if(this.model.get('state') === 'signin') {
			if(!_.isEmail(email)) {
				this.showError(jhst.localize('Doesn&#39;t look like a valid email', 'auth'), 'email')
			} else if (email.length > 50){
				this.showError(jhst.localize('Too much', 'auth'), 'email')
			} else if (_.isEmpty(password)){
				this.showError(jhst.localize('Do you have an empty password?', 'auth'), 'password')
			} else if (password.length > 45){
				this.showError(jhst.localize('Too much', 'auth'), 'password')
			} else {
			...
			if(!_.isEmail(email)) {
				...
				this.showError(jhst.localize('Please enter your name', 'auth'), 'name')
			} else if (name.length > 55){
				this.showError(jhst.localize('Too much', 'auth'), 'name')
			} else if (_.isEmpty(password)){
				this.showError(jhst.localize('Please use not an empty password', 'auth'), 'password')
			} else if (password.length > 45){
				this.showError(jhst.localize('Too much', 'auth'), 'password')
			} else {
	...
	errorSignin: function(error) {
		if(error.status === 401) {
			this.showError(jhst.localize('Wrong e-mail and password combination :(', 'auth'), 'email')
		} else {
			this.showError(jhst.localize('Something went wrong...', 'misc'), 'email')
		}
	},
	...
	showError: function(txt, input_name) {
		...
		switch(input_name) {
			case 'email': 
				this.$input_email.parents('.jhst__inp-cont').toggleClass('error-inp', true);
				this.$input_email.focus();
				break;
			case 'name': 
				this.$input_name.parents('.jhst__inp-cont').toggleClass('error-inp', true);
				this.$input_name.focus();
				break;
			case 'password': 
				this.$input_password.parents('.jhst__inp-cont').toggleClass('error-inp', true);
