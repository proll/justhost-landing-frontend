jhst.Confirm = Backbone.Model.extend({
	url: '/',
	defaults: {
		url: '/',
		data: {},
	...
	initialize: function (options) {
		...
		this.view = new jhst.ConfirmView({model: this});
	},
		...
		if(!response.error) {
			...
			if(!!this.get('event')) {
				...
				if(!!this.get('eventdata') && !_.isEmpty(_.toJSON(this.get('eventdata')))) {
					jhst.trigger(this.get('event'), _.toJSON(this.get('eventdata')));
				} else {
					jhst.trigger(this.get('event'));
				}
			}
		} else {
