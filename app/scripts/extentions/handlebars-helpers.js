...
Handlebars.registerHelper('_', function(phrase, context){
	var phr = phrase;
	if(!!jhst.l10n[context] && !!jhst.l10n[context][phrase]) {
		phr = jhst.l10n[context][phrase][jhst.language]!='' ? jhst.l10n[context][phrase][jhst.language] || jhst.l10n[context][phrase]['en'] || '(;_;)' : '';
	}
...
Handlebars.registerHelper('_plural', function(num, formsJSONStr) {
	var processor = jhst.l10n.plural.processors[jhst.language] || jhst.l10n.plural.processors['en'],
...
Handlebars.registerHelper('_uphoto', function(url, size) {
	if(!url) {
		url = 'http://img.jhstoq.me/photo/SIZEX/764000.jpg';
	}
	return new Handlebars.SafeString(url.replace('SIZEX', size));
});

Handlebars.registerHelper('_prephoto', function(size, id) {
	return new Handlebars.SafeString('http://img.jhstoq.me/photo/'+size+'/'+id+'.jpg');
});
